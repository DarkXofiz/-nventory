...
public class InventoryActionHandler {
    ...
    private static List<Slot> getPlayerSlots(HandledScreen<?> screen, MinecraftClient client) {
        ...
        if (result.isEmpty()) {
            ...
            InventoryMod.LOGGER.info("[InvMod] Player slots found: {}", result.size());
        }
        return result;
    }
    ...
    private static List<Slot> getChestSlots(HandledScreen<?> screen, MinecraftClient client) {
        ...
        if (!hasPlayerSlots && handler.slots.size() >= 36) {
            ...
            InventoryMod.LOGGER.info("[InvMod] Server screen chest slots: {}", result.size());
        }
        return result;
    }
    ...
    private static void dropSlot(HandledScreen<?> screen, MinecraftClient client, Slot slot) {
        ...
        InventoryMod.LOGGER.info("[InvMod] ServerSide slot={} item={} isPV={}", slot.id, before.getItem(), isPersonalVault(screen));
    ...
    public static void putAllToChest(HandledScreen<?> screen, MinecraftClient client) {
        ...
        InventoryMod.LOGGER.info("[InvMod] putAllToChest done (PV/EC supported)");
    }
    ...
    public static void takeAllFromChest(HandledScreen<?> screen, MinecraftClient client) {
        ...
        InventoryMod.LOGGER.info("[InvMod] takeAll: {} slots (PV={})", chestSlots.size(), isPersonalVault(screen));
    ...
    public static void dropAllFromInventory(HandledScreen<?> screen, MinecraftClient client) {
        ...
        InventoryMod.LOGGER.info("[InvMod] dropAll player: {} slots", playerSlots.size());
        for (Slot slot : playerSlots) {
            if (!slot.getStack().isEmpty()) dropSlot(screen, client, slot);
        }
        List<Slot> chestSlots = getChestSlots(screen, client);
        if (!chestSlots.isEmpty()) {
            InventoryMod.LOGGER.info("[InvMod] dropAll chest/PV: {} slots", chestSlots.size());
            for (Slot slot : chestSlots) {
                if (!slot.getStack().isEmpty()) dropSlot(screen, client, slot);
            }
        }
    ...
    private static boolean hasJunkEnchants(ItemStack stack) {
        ...
        // Diamond axe and armor -> never junk
        if (id.contains("diamond_axe"))        return false;
        ...
        for (net.minecraft.registry.entry.RegistryEntry<net.minecraft.enchantment.Enchantment> entry : enchants.getEnchantments()) {
            int lvl = enchants.getLevel(entry);
            String enchId = entry.getIdAsString().toLowerCase();
            if (enchId.contains("protection") && lvl >= 1 && lvl <= 15) return true;
